...
const fetchPosts = async (userId) => {
	...
	while (true) {
		chunkIndex += 1;
		const url = `${API_BASE_URL}/user/clips/chunk?user_id=${userId}${endCursor ? `&end_cursor=${endCursor}` : ''}`;
		...
		// ---- stop conditions (no date cutoff)
		const reachedEnd = !endCursor;
		const hitLimit = posts.length >= POSTS_COUNT_LIMIT;
		const minCount = posts.length > STOP_AFTER_POSTS; // strict ">" to match your ask

		const shouldStop = reachedEnd || hitLimit || minCount;
		...
		// Safety: empty chunk → bail to avoid loops
		if (chunk.length === 0) {
...
const computeStats = (posts) => {
	...
	log.info('AvgPosts/Day window', {
		days: daysForAvg,
		count: windowPostsExclToday.length,
		from: new Date(windowStart).toISOString(),
		to_exclusive: new Date(todayStart).toISOString(),
		avg: avgPostsPerDay,
	});

	// ---- Other stats (unchanged; based on posts ≥ 3 days old)
...
const tooltipEl = () => document.getElementById('igstats-tooltip');
...
const renderActivity = (posts) => {
	...
	// Full width: columns auto-fit to container width
	...
	// Align first day to weekday column (Mon=0)
